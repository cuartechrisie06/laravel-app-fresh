    ...
    <div class="p-8 max-w-4xl mx-auto">
        <div class="bg-slate-900/90 rounded-3xl shadow-xl border border-white/10 overflow-hidden">
            <div class="bg-slate-800/80 px-8 py-6 border-b border-white/10">
                ...
                <p class="text-slate-400 mt-1">Enter all the book details below.</p>
            </div>
            ...
            <div class="p-8 space-y-6">
                ...
                <form method="POST" action="/books" class="grid gap-4 lg:grid-cols-2" enctype="multipart/form-data">
                    @csrf

                    <div>
                        <label class="block text-sm font-semibold text-slate-300">Title</label>
                        <input type="text" name="title" value="{{ old('title') }}" class="mt-1 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-white" required>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-300">Author</label>
                        <input type="text" name="author" value="{{ old('author') }}" class="mt-1 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-white" required>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="published_year" value="{{ old('published_year') }}" class="mt-1 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-white" min="1000" max="2100" required>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-300">ISBN</label>
                        <input type="text" name="isbn" value="{{ old('isbn') }}" class="mt-1 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-white" required>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-300">Number of Pages</label>
                        <input type="number" name="pages" value="{{ old('pages') }}" class="mt-1 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-white" min="1" required>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-300">Language</label>
                        <input type="text" name="language" value="{{ old('language') }}" placeholder="e.g., English" class="mt-1 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-white" required>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-300">Publisher</label>
                        <input type="text" name="publisher" value="{{ old('publisher') }}" class="mt-1 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-white" required>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-300">Price</label>
                        <input type="number" name="price" value="{{ old('price') }}" step="0.01" min="0" class="mt-1 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-white" required>
                    </div>
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <textarea name="description" rows="5" class="mt-1 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-white" required>{{ old('description') }}</textarea>
                    </div>

                    <div class="lg:col-span-2 flex items-center gap-4">
                        <label class="flex items-center gap-3 text-slate-300">
                            <input type="checkbox" name="is_available" value="1" {{ old('is_available') ? 'checked' : '' }} class="w-5 h-5 rounded border-slate-700 bg-slate-950">
